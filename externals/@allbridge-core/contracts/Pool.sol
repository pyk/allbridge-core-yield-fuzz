...
contract Pool is RewardManager {
    ...
    int256 private constant PP = 1e4; // Price Precision
    ...
    uint256 public balanceRatioMinBP;
    uint256 public reserves;
    uint256 public immutable a;
    uint256 public d;

    uint256 private immutable tokenAmountReduce;
    uint256 private immutable tokenAmountIncrease;

    // can restrict deposit or withdraw operations
    address private stopAuthority;
    // is deposit operation allowed
    uint256 public canDeposit = 1;
    // is withdraw operation allowed
    uint256 public canWithdraw = 1;
        ...
        RewardManager(token_, lpName, lpSymbol)
    {
        ...
        uint256 decimals = token_.decimals();
        tokenAmountReduce = decimals > SYSTEM_PRECISION
            ? 10 ** (decimals - SYSTEM_PRECISION)
            : 0;
        tokenAmountIncrease = decimals < SYSTEM_PRECISION
            ? 10 ** (SYSTEM_PRECISION - decimals)
            : 0;
    }
    ...
    modifier validateBalanceRatio() {
        ...
        if (tokenBalance > vUsdBalance) {
            require(
                (vUsdBalance * BP) / tokenBalance >= balanceRatioMinBP,
                "Pool: low vUSD balance"
            );
        } else if (tokenBalance < vUsdBalance) {
            require(
                (tokenBalance * BP) / vUsdBalance >= balanceRatioMinBP,
     ...
     */
    function deposit(uint256 amount) external whenCanDeposit {
        uint256 oldD = d;

        uint256 amountSP = _toSystemPrecision(amount);
        ...
        returns (uint256)
    {
        ...
        if (amount > 0) {
            ...
            uint256 amountIn = _toSystemPrecision(amount - fee);
    ...
    function getY(uint256 x) external view returns (uint256) {
        ...
        uint256 a8 = a4 << 1;
        // 4A(D - x) - D
        int256 part1 = int256(a4) * (int256(d_) - int256(x)) - int256(d_);
        // x * (4AD³ + x(part1²))
        uint256 part2 = x * (a4 * d_ * d_ * d_ + x * uint256(part1 * part1));
        // (sqrt(part2) + x(part1)) / 8Ax)
        return SafeCast.toUint256(int256(_sqrt(part2)) + int256(x) * part1)
            / (a8 * x) + 1; // +1 to offset rounding errors
    }
    ...
    function getPrice() external view returns (uint256) {
        ...
        uint256 a8 = a << 3;
        uint256 dCubed = d * d * d;

        // 4A(D - x) - D
        int256 p1 = int256(a << 2) * (int256(d) - int256(x)) - int256(d);
        // x * 4AD³ + x(p1²)
        uint256 p2 = x * ((a << 2) * dCubed + x * uint256(p1 * p1));
        // D³ + 8ADx² - 8Ax³ - 2Dx²
        int256 p3 = int256(dCubed) + int256((a << 3) * d * x * x)
            - int256(a8 * x * x * x) - int256((d << 1) * x * x);
        // 1/2 * p3 / (4x * sqrt(p2))
        return SafeCast.toUint256(
            (PP >> 1) + ((PP * p3) / int256((x << 2) * _sqrt(p2)))
        );
    }
    ...
    function _toSystemPrecision(uint256 amount)
        internal
        view
        returns (uint256)
    {
    ...
    function _fromSystemPrecision(uint256 amount)
        internal
        view
        returns (uint256)
    {
